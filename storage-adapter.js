...
document.addEventListener('click', (event)=>{
  const nav = event.target.closest('[data-nav="faenas"]');
  if(!nav || typeof window.amecoOpenSites !== 'function') return;
...
const TAB_TITLES = {dashboard:'Resumen general', trabajadores:'Trabajadores', faenas:'Faenas', documentos:'Documentos y exámenes', acreditaciones:'Acreditación por faena', certificaciones:'Certificaciones por equipo', turnos:'Turnos 14x14'};
...
function workerModalHtml(workerId){
  ...
  return '<div class="modal-overlay" data-action="close-modal-overlay"><div class="modal">'+
    ...
    '<div class="modal-body" id="workerForm" data-worker-id="'+w.id+'" data-is-new="'+isNew+'">'+
      ...
      '<div class="field-row">'+
        '<div class="field"><label>RUT</label><input id="f-rut" class="mono" value="'+escAttr(w.rut)+'" placeholder="Ej: 20.910.828-3"></div>'+
        '<div class="field"><label>Cargo</label><select id="f-cargo">'+state.cargos.map(c=>'<option value="'+c+'" '+(w.cargo===c?'selected':'')+'>'+c+'</option>').join('')+'</select><div class="helptext">Selecciona uno de los cargos configurados en el sistema.</div></div>'+
      '</div>'+
      '<div class="field-row">'+
      ...
      '</div>'+
      '<div class="field-row">'+
        '<div class="field"><label>Ciudad</label><input id="f-ciudad" value="'+escAttr(w.ciudad)+'" placeholder="Ej: Antofagasta"></div>'+
        '<div class="field"><label>Empresa / contratista (opcional)</label><input id="f-empresa" value="'+escAttr(w.empresa||'')+'" placeholder="Ej: AMECO"></div>'+
      '</div>'+
...
document.addEventListener('click', async (e)=>{
  ...
  if(!el){
    ...
    if(navEl){
      ...
      window.amecoCurrentModule = null;
      render();
    }
    return;
  }
