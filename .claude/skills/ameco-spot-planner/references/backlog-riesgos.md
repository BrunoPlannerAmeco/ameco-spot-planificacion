...
|---|---|---|---|---|---|
| CHK-01 | Roles y reglas | **Gap de seguridad real cerrado** (verificado en código y probado en producción): 3 roles (admin/planner/viewer) con enforcement server-side. Lector no puede escribir nada (RTDB). Planificador no puede eliminar trabajadores — antes solo se ocultaba el botón en el cliente; ahora `amecoSpotPlanner/workers/{id}` es un nodo real con regla propia (Planificador crea/edita, no elimina) y `index.html` pasa por ahí antes de tocar `legacyStorage`, probado con `@firebase/rules-unit-testing` en CI y en vivo contra producción (Planificador rechazado, Admin permitido). No fue necesario esperar a CHK-06 completo: se resolvió con una "cuña" (un nodo normalizado por entidad, no todo el modelo). **Pendiente, no bloqueante**: el mismo patrón no se replicó todavía para eliminar cargos/faenas (siguen solo con gate de cliente); tampoco hay custom claims (rol se lee de RTDB, no del token) ni desactivación en tiempo real (el usuario desactivado sigue con sesión activa hasta reload/refresh de token) | Replicar el patrón de `workers/{id}` para cargos/faenas si se prioriza; listener en vivo de `active` para forzar logout inmediato; evaluar migración a custom claims | Lector no puede escribir (cumplido, server-side); Admin/Planificador según matriz para trabajadores (cumplido, server-side); para cargos/faenas (pendiente, solo cliente) | Crítica |
