...
| CHK-06 | Normalizar RTDB | **Fase 1 en marcha**: `equipos` es la primera entidad con normalización real (no solo "cuña" de permisos como `workers`/`cargosCatalog`/`faenasCatalog`, que siguen siendo espejos de solo escritura que la UI nunca lee de vuelta). `amecoSpotPlanner/equiposCatalog/{key}` es ahora la fuente de verdad: la app lee en vivo con un listener (`amecoAccessApi.subscribeEquiposCatalog`), escribe con `set()` granular por clave (no reescribe el blob), y respaldo/restauración quedan sincronizados (`syncEquiposCatalogFromRestore`). De paso cierra un gap tipo CHK-01: antes `equipos` solo estaba oculto a Planificador en el cliente, ahora la regla exige Admin server-side. El resto de las 8 entidades del payload (`workers, faenas, cargos, cargoColores, llamados, pasajes, servicios`) sigue en el blob único `legacyStorage`: **cada guardado, hasta cambiar un día de turno de un trabajador, sigue reescribiendo el JSON completo** (`persist()`/`persistNow()` → `buildPersistentPayload()`) | Repetir el mismo patrón (nodo real + regla + listener en vivo + escritura granular + respaldo sincronizado) para `llamados` y `pasajes` (siguiente escalón: estructuras planas, ~20 puntos de mutación cada una); después `cargosCatalog`/`faenasCatalog` (ya tienen el nodo escrito, falta voltear la lectura); `servicios` es más complejo por sus sub-arrays; los overrides de turnos dentro de cada `worker` son el caso más delicado (alta frecuencia de escritura concurrente) | Ediciones concurrentes no sobrescriben módulos — cumplido para `equipos`; pendiente para el resto | Crítica |
...
4. **CHK-06 Normalización** — bloquea sincronización y escalabilidad general (ya no bloquea CHK-01: se resolvió con una cuña puntual en `workers/{id}` sin esperar la normalización completa). Fase 1 (`equipos`) ya demuestra el patrón completo de normalización real; faltan `llamados`, `pasajes`, el flip de lectura de `cargosCatalog`/`faenasCatalog`, `servicios` y los overrides de turnos.
